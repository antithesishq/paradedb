...
# eventually command starts, but the database can still be mid-recovery, so the deadline loop
# rides out connection errors as well as pg_search background activity racing the checker
# (paradedb/paradedb#5913); real corruption fails every attempt until the deadline.
...
while (( SECONDS < DEADLINE )); do
  ...
  echo "verify_index retry: ${output}" >&2
  sleep 5
done
